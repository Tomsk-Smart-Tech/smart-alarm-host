    ...
    : QObject{parent}
{
    ...
    m_cur_photo=read_user_json("wallpaper");
}
...
Q_INVOKABLE void LinuxTerminal::scanPhotos(QString path)
{
    m_photos.clear();
    QDirIterator it(path, QStringList() << "*.jpg" << "*.jpeg" << "*.png", QDir::Files, QDirIterator::Subdirectories);
    while (it.hasNext()) {
        QVariantMap map;
        QString filePath = it.next();
        qDebug()<<filePath;
        QString fileName = QFileInfo(filePath).fileName();
        map["photoPath"] = QUrl::fromLocalFile(filePath).toString();
        map["photoName"] = fileName;
        m_photos.append(map);
    }
...
Q_INVOKABLE void LinuxTerminal::set_photo(QString path)
{
    if(m_cur_photo!=path)
    {
        m_cur_photo=path;
        write_user_json("wallpaper",path);
